...
class Tester():  # Evaluation code
    ...
    def _build_dataset(self, tl, dpath, args):
        ...
        if tl < 1:  # For ratio case, find corresponding references and input articles
            # Gigaword contains a single reference set
    ...
    def get_score(self, model, aid):  # aid: ID to select a proper dataset
        ...
        with torch.no_grad():
            ...
            for idx, bi in enumerate(binputs):
                ...
                batch_target_lens = target_lens[batch_size *
            ...
            # truncate summaries only for DUC2004
            if len(set(target_lens)) == 1 and target_lens[0] == 13:
            ...
            # Get ROUGE scores
            scores = self.evaluator.get_score(
                trunc_preds, arts, refers, rouge_type)
            ...
            fluscore = self.evaluator.lm_model.get_lm_score(
                trunc_preds).cpu().numpy()
            semscore = self.evaluator.ss_model.get_ss_score(
                trunc_preds, arts).cpu().numpy()

            length_score = length_reward(
                trunc_preds, target_lens).cpu().numpy()
...
def train(args, train_dataset, model, tokenizer) -> Tuple[int, float]:
    ...
    tester = Tester(args, tokenizer, ss_model, lm_model,
                    args.target_len)  # Evaluation function
        ...
        if args.isrange == False:  # Fixed length case
            for tl in args.target_len:
                ...
                for tid, tlen in examples:  # Num data x target length
                    ...
                    if tl >= 1:  # integer target lengths
                        ...
                        # tlen: true length, tl: target length
                        cl = max(int(tlen * tl), 1)

                    prefix = '{}: '.format(cl)  # cl: current length

                    ids_prefix = tokenizer(
            ...
            for _ in range(args.num_msl):
                for tid, tlen in examples:  # Num data x target length
                    ...
                    # maxlen+1: below randint sample from [min, max)
                    each_range = [minlen, min(maxlen+1, tlen)]
                    # Sample a length regardless of input length!
                    cl = np.random.randint(each_range[0], each_range[1])
    ...
    if args.isrange == False:
        ...
        print('  Target Length Range = {} ~ {}'.format(
            min(args.target_len), max(args.target_len)))
    ...
    allsteps = len(train_dataloader) * \
        int(args.num_train_epochs)  # total number of batches
    ...
    for trn_iter in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for i in range(args.num_agent):
                ...
                # e.g., [8, ..., 8, 10, ..., 10, 13, ..., 13]
                target_length = alllen

                st = time.time()

                # Maximum and minimum number of tokens
                # preparing 3 tokens per word (from data analysis)
                curmaxlen = int(target_length.max() * 3)
                ...
                if args.lead == True:  # Use lead bias as baseline in reinforcement learning
                    tk_greedy_sents = [' '.join(tk_inputs[i].split(
                    )[:target_length[i]]) for i in range(len(target_length))]
                else:
                ...
                with torch.no_grad():
                    lbl_weight = min(
                        (global_step+1)/args.lbl_maxstep+args.lbl_min, 1)
                ...
                confidence = sample_all_rw[1] * sample_all_rw[2]

                # len(target lengths) * batch_size
                all_samples = np.array(
                    tk_sample_sents).reshape(-1, args.batch_size_trn)
                all_greedys = np.array(
                    tk_greedy_sents).reshape(-1, args.batch_size_trn)
                all_confidences = confidence.reshape(-1, args.batch_size_trn)

                all_each_reward.append(torch.stack(
                    sample_all_rw).mean(-1).cpu().numpy())
                ...
                all_length = np.array(
                    target_length).reshape(-1, args.batch_size_trn)
            ...
            for aid in range(args.num_msl):
                ...
                for oaid in range(args.num_msl):
                    if oaid == aid:
                        continue  # exclude self-reference case

                    sample_consis_reward = rg[0].get_ss_score(
                        all_samples[aid], all_samples[oaid])
                    baseline_consis_reward = rg[0].get_ss_score(
                        all_greedys[aid], all_samples[oaid])
                    ...
                    len_weight = (-torch.Tensor(lendiff/args.scl)
                                  ).exp().cuda()  # length reward

                    weight = len_weight * conf_weight.pow(args.alpha)

                    sample_consistencies.append(weight * sample_consis_reward)
                    baseline_consistencies.append(
                        weight * baseline_consis_reward)

                consistency_reward_sample = torch.stack(
                    sample_consistencies).mean(0)
                consistency_reward_baseline = torch.stack(
                    baseline_consistencies).mean(0)

                sample_consistencies_4eachlen.append(consistency_reward_sample)
                baseline_consistencies_4eachlen.append(
                    consistency_reward_baseline)

            sample_consistencies_4eachlen = torch.cat(
                sample_consistencies_4eachlen)
            baselin_consistencies_4eachlen = torch.cat(
                baseline_consistencies_4eachlen)

            # Original rewards + the reward from multi-summary learning mechanism
            sample_reward = all_sample_rewards[0] + \
                args.lb * sample_consistencies_4eachlen
            baseline_reward = all_baseline_rewards[0] + \
                args.lb * baselin_consistencies_4eachlen
                ...
                if args.logging_steps > 0 and global_step % args.save_steps == 0:
                    if args.evaluate_during_training:
                        ...
                        with torch.no_grad():
                            all_tar_scores = []
                            all_reward_product = []
                            # 3: [8, 10, 13] lengths, 1: [50%] length
                            NUM_EVAL_CASE = 3 if args.islength else 1
                            for eid in range(NUM_EVAL_CASE):
                                scores, reward_product = tester.get_score(
                                    model, eid)
                                all_tar_scores.append(scores)
                                all_reward_product.append(reward_product)

                        all_reward_product = np.stack(
                            all_reward_product).mean()

                        tar_rouge1, tar_rouge2, tar_rougeL, lenerr, rflu, rsem, _ = np.array(
                            all_tar_scores).mean(0)  # aggregate each reward
...
#                         eval_criterion = all_reward_product
                        ...
                        if args.islength == True:
                            for tlidx, tl4log in enumerate([8, 10, 13]):
                                eval_log['Eval/Avg len {}'.format(
                                    tl4log)] = best_avg_len[tlidx]
                        else:
                            eval_log['Eval/Avg len {}'.format(
                                '50%')] = best_avg_len[0]
            ...
            curepoch = (len(train_dataloader) * trn_iter + step) / \
                allsteps * args.num_train_epochs
            epochstr = '{:.2}/{}'.format(curepoch, int(args.num_train_epochs))

            inc_batch_loss = inc_batch_loss + \
                (agentloss/args.num_agent - inc_batch_loss)/(step+1)
...
def main():
    ...
    parser.add_argument(
        "--train_file", default='train.article.txt', type=str, required=False)

    parser.add_argument("--learning_rate", default=5e-5, type=float)
    parser.add_argument("--weight_decay", default=1e-2, type=float)
    parser.add_argument("--lb", default=0.01, type=float,
                        help='Weight for the quality reward R_C')
    parser.add_argument("--lbs", default=1, type=float,
                        help='Weight for semantic-similarity reward R_S')
    parser.add_argument("--lbf", default=1, type=float,
                        help='Weight for fluency reward R_F')
    parser.add_argument("--lbl", default=1, type=float,
                        help='Weight for length reward R_L')
    ...
    parser.add_argument("--overlap", default=False,
                        type=str2bool, help='Weight for length reward R_L')
    parser.add_argument("--lead", default=False, type=str2bool,
                        help='Whether to use lead bias as a baseline')

    parser.add_argument("--scl", default=10, type=float,
                        help='scale for length reward')
    parser.add_argument("--lendecay", default=1, type=float,
                        help='scale for length reward')
    parser.add_argument("--scf", default=1000, type=float,
                        help='scale for fluency reward')

    parser.add_argument('--num_agent', type=int, default=1,
                        help="# of agents")  # Deprecated
    parser.add_argument('--ptype', default='t5-pretrain/tl20_row1.0_rs0.1_rd0.1_nc1',
                        type=str, help="type of pretrained model")
    parser.add_argument('--no_repeat_ngram', type=int,
                        default=3, help="No repeatition for the n-gram")
    parser.add_argument('--repeat_penalty', type=float,
                        default=1.0, help="Weight as a penalty")

    parser.add_argument("--target_len", default='8,10,13',
                        help="Target lengths")
    parser.add_argument("--num_msl", default=3, type=int,
                        help="# branches of multi-summary learning")

    parser.add_argument("--numdata", default=500000,
                        type=int, help="# of training data to use")
    parser.add_argument("--numvlddata", default=500,
                        type=int, help="# of validation data")
    ...
    parser.add_argument("--batch_size_trn", default=24,
                        type=int, help="Batch size for trn")
    parser.add_argument("--batch_size_eval", default=24,
                        type=int, help="Batch size for eval")
    ...
    parser.add_argument("--evaluate_during_training", default=True,
                        action="store_true", help="Eval during Trn at each logging step")
    ...
    parser.add_argument("--patient", default=10, type=int,
                        help="# patient steps before early stop")
    ...
    parser.add_argument("--warmup_steps", default=0,
                        type=int, help="Linear warmup steps.")

    parser.add_argument("--overwrite_output_dir", default=True,
                        action="store_true", help="Overwrite output dir")
    ...
    parser.add_argument('--wandb', default='disabled',
                        type=str)  # disabled or online
